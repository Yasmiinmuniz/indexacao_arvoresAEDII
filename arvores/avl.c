...
avl adicionar_avl(avl a, indice_avl *novoIndice, int *cresceu) {
	...
	//CASO BASE: raiz vazia, então cria um novo no.
	if(a == NULL) {
		avl new = (avl) malloc(sizeof(no_avl));
		new->dado = novoIndice;
		new->esq = NULL;
		new->dir = NULL;
		new->fb = 0; //fb é o fator de balanceamento
		*cresceu = 1;
		...
		//se valor > raiz, adicione o elemento a direita
		if(novoIndice->codigo > a->dado->codigo) {
			a->dir = adicionar_avl(a->dir, novoIndice, cresceu);

			//calcular os reajustes do fator de balanco
			if(*cresceu) {
				switch(a->fb){
					//arvore totalmente balancada, agora tem um fator no lado direito
					case 0:
						a->fb = 1;
					   *cresceu = 1;
					    break;
					
					//arvore estava pendendo pra esquerda, agora ficou equilibrada
					case -1:
						a->fb = 0;
						*cresceu = 0;
						break;

					//caso o fb no lado direito 1 e agora se torne 2, logo precisa ser balanceada
					case 1:
						*cresceu = 0; 
						a = rotacao_avl(a);//pendeu para direita, rota para esquerda
						break;
			...
			//elemento < raiz, adicionar elemento esquerda
			a->esq = adicionar_avl(a->esq, novoIndice, cresceu);

			//calcular os reajustes do fator de balanco
			if(*cresceu) {
				switch(a->fb) {
					//arvore balanceada e agora pende 1 para direita
					case 0:
						a->fb = -1;
						*cresceu = 1;
						break;
					//arvore pende 1 para esquerda e agora equilibra
					case 1:
						a->fb = 0;
						*cresceu = 0;
						break;

					//pendia 1 para esquerda e agora desbalanceia
					case -1:
						//a->fb = -2;
						*cresceu = 0;
						a = rotacao_avl(a);
  	...
  	if(a->dado->codigo == v) {
			...
			if(v > a->dado->codigo) {
		    		...
		    		if(*diminuiu) 
		    		{
		    			switch(a->fb)
		    			{	
		    				//raiz pendendo esquerda, agora que removeu ela, gerou -2. Ou seja
		    					//rotacao_avl
		    				case -1:
		    					a->fb = -2;
		    					if(a->esq->fb == 0)
		    				...
		    				//raiz balanceadaaaaaaaa, agora pende pra esquerda
		    				case 0:
		    					a->fb = -1;
		    					*diminuiu = 0;
		    					break;
		    		...
		    		if(*diminuiu) 
		    		{
		    			switch(a->fb) 
		    			{
		    				//raiz estava pendendo pra esquerda, agora equilibrou
		    				case -1:
		    					a->fb = 0;
		    					*diminuiu = 1;
		    					break;
